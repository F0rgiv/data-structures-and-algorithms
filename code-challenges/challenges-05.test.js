...
const sortStarWarsCharacters = (starWarsArr) => {
  // sort contents of arr by .height
  starWarsArr.sort((l, r) => {
...
const removeThree = (idx, arr) => {
  // remove 3 starting at index spesified
  arr.splice(idx, 3);
  return arr;
};
...
const howMuchPencil = (str) => {
  let result = [];
  // loop through str ad append to the result
  let i = 0
...
const wordsToCharList = (arr) => { //arr should be named str?
  // spli in each char
  return arr.split('');
};
...
const listFoods = (recipe) => {
  //solution assumes format is always '[quantety] [mesurment] [ingerdiant]'
  let result = [];
...
const splitFoods = (recipe) => {
  ...
  // loop through recipy
  recipe.ingredients.forEach(str => {
    //get array of words and remove first two.
    let words = str.split(' ');
    //drop ingrediant amount
    words.shift()
    //drop ingerdiant mesurmant
    words.shift()

    // regoin words and push to arr
...
const removeEvenValues = (arr) => {
  // get legth of arr as filter creates a new arr:( 
  let i = arr.length;

  // start at end of Array, if you start at the and end remove as you go things will brake as what was index 1 will not be 0.
  while (i--) {
    if (arr[i] % 2 === 0) {
      arr.splice(i, 1);
    }
