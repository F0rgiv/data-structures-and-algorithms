...
const longestString = (arr) => {
  //return -1 of empty and 0 if only a single one
  if (arr.length < 1) {
    return -1;
  ...
  // Convert to lengths
  const lagthsArr = arr.map(str => str.length);
  // return largest
  return lagthsArr.reduce((index1, str, index2) => lagthsArr[index1] > lagthsArr[index2] ? index1 : index2);
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map(phonenumber => phonenumber.match(/\d/g).join(''))
};
...
const onlyOddChars = (str) => {
  // Split to array and filter out non odd index adn then join back together
...
const allHappy = (arr) => {
  // Solution code here...
  const boolArr = arr.map(str => /:\)/.test(str));
  //set result and change to fase if found
  let result = true;
...
const findAnything = (arr, target) => {
  // Solution code here...
  return arr.filter(str => !(str.indexOf(target) == -1))
};
...
const findEvery = (arr, target) => {
  // Solution code here...
  const boolArr = arr.map(str => !(str.indexOf(target) == -1));
  //set result and change to fase if found
  let result = true;
  boolArr.forEach(element => {
    if (!(element)) {
...
const unenrollBrook = (arr) => {
  // Map through arrays and run reduce to clean out any that are like Brook
  return arr.map(innerArr => innerArr.reduce((arr, str) => {
    /Brook/.test(str) ? arr : arr.push(str);
    return arr;
  }, []));
};
...
const characterByIndex = (arr) => {
  // Solution code here...
  return arr.map((subArr, index) => subArr[index]);
};
