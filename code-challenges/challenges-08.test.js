...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  // Only return base stat greater than minBaseStat
...
const getStatName = (arr, minBaseStat) => {
  // Only get base stat greater than minBaseStat
  const filteredStats = arr.filter(stat => stat.baseStat > minBaseStat)
  // only return the stat name
...
const getCharactersWithoutChildren = (arr) => {
  // only return houses with no children
...
const evenOddNumericValues = (arr) => {
  ...
  // Make nums into 'even' or 'odd'
  return nums.map(num => num % 2 ? "odd" : "even") // ? is not needed can use a more simple if for readability but practicing.
};
