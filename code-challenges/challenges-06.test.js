...
const checkValues = (obj, value) => {
  ...
  //check if value does not exist and return false if it doesn't
...
const hasChildrenValues = (arr, character) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    const family = arr[i];
    const vals = Object.values(family);

    //see if the wanted charactor is in the obj
    if (!(vals.indexOf(character) === -1)) {
      // return legth of childeren as a Boolean(len 0 will be false)
      return !!vals[2].length;
    };
  };
  //catch any that aren't in the list
  return false;
};
...
const hasChildrenEntries = (arr, character) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    const family = arr[i];
    const vals = Object.entries(family);

    //see if the wanted charactor is in the obj
    if (vals[0][1] == character || vals[1][1] == character) {
      // return legth of childeren as a Boolean(len 0 will be false)
      return !!vals[2][1].length;
    };
  };
  //catch any that aren't in the list
  return false;
...
const totalCharacters = (arr) => {
  // count up the mambers
  let count = 0;
  arr.forEach(family => {
    //for the family head
    count++;
    //spuce if applicable
    if (family.spouse) { count++ };
    //add children
...
const houseSize = (arr) => {  //confused it's expecting an Object?
  const sizes = [];
  ...
  arr.forEach(family => {
    //get members
    if (family.spouse) {
      count = (2 + family.children.length)
    } else {
      count = (1 + family.children.length)
    }
    //push the house name and member count to the array
    sizes.push({
      house: family.house,
      members: count
    })
  });
  // return complere array
  return sizes;
};
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach(family => {
    ...
    // dount count the spouse if they are in the list
    if (family.spouse && deceasedSpouses.indexOf(family.spouse) === -1) {
      count = (2 + family.children.length)
    } else {
      count = (1 + family.children.length)
    };
